/**
 * MCP Server function for RollbackApiDeployment sets the current revision to a specified prior
...
class Post_V1_Projects_Project_Locations_Location_Apis_Api_Deployments_Deployment_RollbackMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_V1_Projects_Project_Locations_Location_Apis_Api_Deployments_Deployment_RollbackHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("name")) {
            queryParams.add("name=" + args.get("name"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_v1_projects_project_locations_location_apis_api_deployments_deployment_rollback" + queryString;
    ...
    public static MCPServer.Tool createPost_V1_Projects_Project_Locations_Location_Apis_Api_Deployments_Deployment_RollbackTool(MCPServer.APIConfig config) {
        ...
        revisionIdProperty.put("description", "Input parameter: Required. The revision ID to roll back to. It must be a revision of the same deployment. Example: c7cfa2a8");
        ...
        nameProperty.put("description", "Input parameter: Required. The deployment being rolled back.");
        properties.put("name", nameProperty);
        parameters.put("properties", properties);
        
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_v1_projects_project_locations_location_apis_api_deployments_deployment_rollback",
            "RollbackApiDeployment sets the current revision to a specified prior
 revision. Note that this creates a new revision with a new revision ID.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_V1_Projects_Project_Locations_Location_Apis_Api_Deployments_Deployment_RollbackHandler(config));
    }
