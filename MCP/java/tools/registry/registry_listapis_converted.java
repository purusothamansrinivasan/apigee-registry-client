/**
 * MCP Server function for ListApis returns matching APIs.
 */
...
class Get_V1_Projects_Project_Locations_Location_ApisMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_V1_Projects_Project_Locations_Location_ApisHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("pageSize")) {
            queryParams.add("pageSize=" + args.get("pageSize"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_v1_projects_project_locations_location_apis" + queryString;
    ...
    public static MCPServer.Tool createGet_V1_Projects_Project_Locations_Location_ApisTool(MCPServer.APIConfig config) {
        ...
        pageTokenProperty.put("description", "A page token, received from a previous `ListApis` call. Provide this to retrieve the subsequent page. When paginating, all other parameters provided to `ListApis` must match the call that provided the page token.");
        ...
        pageSizeProperty.put("description", "The maximum number of APIs to return. The service may return fewer than this value. If unspecified, at most 50 values will be returned. The maximum is 1000; values above 1000 will be coerced to 1000.");
        properties.put("pageSize", pageSizeProperty);
        parameters.put("properties", properties);
        
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "get_v1_projects_project_locations_location_apis",
            "ListApis returns matching APIs.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getGet_V1_Projects_Project_Locations_Location_ApisHandler(config));
    }
